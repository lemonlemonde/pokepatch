-- PokePatch: order quotes (line items + bulk counts + single override)
--
-- SAFETY (additive / non-destructive to existing order data)
-- - No DROP TABLE / DROP COLUMN / TRUNCATE.
-- - No DROP FUNCTION (keeps the existing update_order(uuid,jsonb,jsonb,jsonb) signature).
-- - Never touches quote_requests or *_original tables.
-- - Only ADDITIVE schema: nullable columns on orders + new order_quote_items table.
-- - create_order unchanged; existing orders keep working with empty quotes.
-- - Wrapped in a transaction (all-or-nothing).
--
-- Note: update_order may replace quote lines for a single order_id when an admin
-- saves a quote. That only affects public.order_quote_items for that order.
...
alter table public.orders
  add column if not exists quote_bulk_counts jsonb,
  add column if not exists quote_override_label text,
  add column if not exists quote_override_amount numeric(10, 2);
...
create index if not exists order_quote_items_order_id_idx
  on public.order_quote_items (order_id, sort_order);

alter table public.order_quote_items enable row level security;

-- Same signature as existing update_order — CREATE OR REPLACE only (no DROP).
...
begin
  ...
  if p_order is not null and jsonb_typeof(p_order) = 'object' then
    ...
    -- Quote line replace for this order only (admin save). Key omitted = leave unchanged.
    if p_order ? 'quote_items' then
      ...
      -- Scoped to this order_id only; does not touch orders/cards/contacts.

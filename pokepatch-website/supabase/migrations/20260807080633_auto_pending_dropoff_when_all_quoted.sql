-- When every card on a pending-quote order has a priced quote line, move it to pending drop-off.

CREATE OR REPLACE FUNCTION public.order_all_cards_have_quotes(p_order_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT
    EXISTS (SELECT 1 FROM public.cards c WHERE c.order_id = p_order_id)
    AND NOT EXISTS (
      SELECT 1
      FROM public.cards c
      WHERE c.order_id = p_order_id
        AND NOT EXISTS (
          ...
          WHERE qi.order_id = p_order_id
            AND lower(trim(qi.card_name)) = lower(trim(c.card_name))
            AND lower(trim(coalesce(qi.set_name, ''))) = lower(trim(coalesce(c.set_name, '')))
            AND trim(coalesce(qi.service_key, '')) <> ''
            AND trim(coalesce(qi.service_label, '')) <> ''
        )
    );
$$;

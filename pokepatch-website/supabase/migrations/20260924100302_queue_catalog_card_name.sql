-- Official catalog title/set for the public queue (from Pokémon TCG API apply).
-- Customer-entered cards.card_name / set_name stay untouched for the order itself.

ALTER TABLE public.cards
  ADD COLUMN IF NOT EXISTS catalog_card_name text,
  ADD COLUMN IF NOT EXISTS catalog_set_name text;

COMMENT ON COLUMN public.cards.catalog_card_name IS
  'Official Pokémon TCG API card name for the public queue; null falls back to cards.card_name.';
COMMENT ON COLUMN public.cards.catalog_set_name IS
  'Official Pokémon TCG API set name for the public queue; null falls back to cards.set_name.';

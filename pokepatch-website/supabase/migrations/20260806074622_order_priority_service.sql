-- Paid priority service: $25 base + $10 per extra card (whole order).
-- Priority orders sort ahead in the To do queue.

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS is_priority boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.orders.is_priority IS
  'Paid priority service — whole order jumps ahead in the To do queue.';
...
BEGIN
  ...
  IF v_status = 'new' THEN
    SELECT COALESCE(array_agg(id ORDER BY is_priority DESC, queue_priority ASC NULLS LAST, created_at ASC NULLS LAST, id ASC), '{}')
      INTO v_ids
    FROM public.orders
    WHERE status = 'new';

-- Per-card admin notes: separate from the customer's card description and
-- from the order-level general_notes. Customers can view it (read-only via
-- get_my_order); only update_order (service_role / admin-api) can write it.
  ...
  if p_cards is not null then
    ...
    loop
      ...
      if v_card_id is not null and exists (
        ...
        if v_card ? 'admin_notes' then
          update public.cards
          set admin_notes = nullif(trim(coalesce(v_card ->> 'admin_notes', '')), '')
          where id = v_card_id;
        end if;
        ...
        v_admin_notes := nullif(trim(coalesce(v_card ->> 'admin_notes', '')), '');

-- Restore order-level Google Drive folder URLs (rollback of cards.photos_drive_url).
-- Backfills from the first non-empty card Drive URL per order (by card.id),
-- verifies, restores RPCs, then drops cards.photos_drive_url.

alter table public.orders
  add column if not exists photos_drive_url text;

update public.orders o
set photos_drive_url = s.drive_url
from (
  select distinct on (c.order_id)
    c.order_id,
    nullif(trim(c.photos_drive_url), '') as drive_url
  from public.cards c
  where c.photos_drive_url is not null
    and trim(c.photos_drive_url) <> ''
  order by c.order_id, c.id
) s
where o.id = s.order_id
  and (o.photos_drive_url is null or trim(o.photos_drive_url) = '');

-- Fail (and roll back) if any card Drive URL was not copied onto its order.
do $$
declare
  v_missing int;
begin
  ...
  if v_missing > 0 then
    raise exception
      'restore_orders_photos_drive_url backfill incomplete: % card(s) with Drive URL but empty order URL',
      v_missing;
  end if;
end;
$$;
...
-- Safe to drop only after verified backfill and RPC restore above (same transaction).

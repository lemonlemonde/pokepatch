-- Priority service: flat $15 per card for new orders (was $25 first + $10 each additional).
-- Existing quote_bulk_counts Priority adjustments are left unchanged.
-- Shared helper keeps create_order and future callers in sync.
...
PARALLEL SAFE
AS $$
  SELECT (GREATEST(1, COALESCE(p_card_count, 1)) * 15)::numeric(10, 2);
$$;

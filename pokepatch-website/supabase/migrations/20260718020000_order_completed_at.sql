-- PokePatch: track when orders were completed (for 7-day visibility)
--
-- Adds orders.completed_at, sets/clears it from update_order when status
-- changes, and exposes it on get_my_orders.
--
-- SAFETY
-- - Additive column + function replaces only.
...
-- Existing completed orders: start the 7-day window from now so nothing
-- suddenly disappears the moment this migration is applied.
...
begin
  ...
  if p_order is not null and jsonb_typeof(p_order) = 'object' then
    ...
    if p_order ? 'status' then
      ...
      if v_status not in ('new', 'in_progress', 'completed') then
      ...
      set
        status = v_status,
        completed_at = case
          when v_status = 'completed' and v_prev_status is distinct from 'completed'
            then now()

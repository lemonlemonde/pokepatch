-- Ignore canceled cards when auto-advancing order to ready (match frontend).
-- Active cards all completed → ready; canceled cards do not block.
...
begin
  ...
  -- all active (non-canceled) cards completed → ready; else any in_progress → in_progress.

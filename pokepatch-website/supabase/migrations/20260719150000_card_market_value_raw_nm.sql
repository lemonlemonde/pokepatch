-- PokePatch: per-card Raw Near Mint market value (admin-set)
--
-- SAFETY (additive)
-- - No DROP TABLE / DROP COLUMN / TRUNCATE.
-- - CREATE OR REPLACE update_order / get_my_order only (same signatures).
-- - Nullable column; existing cards keep NULL.
...
begin
  if to_regclass('public.cards') is null then
    raise exception 'Refusing to apply: public.cards is missing.';
  end if;
end;
$$;

alter table public.cards
  add column if not exists market_value_raw_nm numeric(10, 2);

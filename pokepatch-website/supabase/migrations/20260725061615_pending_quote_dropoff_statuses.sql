-- Pending column: rename on_hold → pending, add pending_kind (quote | drop_off).
-- One status drives the kanban column; kind is the chip/customer label.

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS pending_kind text;

ALTER TABLE public.orders DROP CONSTRAINT IF EXISTS orders_pending_kind_check;
ALTER TABLE public.orders
  ADD CONSTRAINT orders_pending_kind_check
  CHECK (
    pending_kind IS NULL
    OR pending_kind = ANY (ARRAY['quote'::text, 'drop_off'::text])
  );
...
-- Legacy / interim statuses → pending + kind.
UPDATE public.orders
SET
  status = 'pending',
  pending_kind = CASE
    WHEN status = 'pending_dropoff' THEN 'drop_off'
    ELSE 'quote'
  END
WHERE status IN ('on_hold', 'pending_quote', 'pending_dropoff', 'pending');

UPDATE public.orders
SET pending_kind = 'quote'
WHERE status = 'pending' AND pending_kind IS NULL;
...
ALTER TABLE public.orders DROP CONSTRAINT IF EXISTS orders_pending_kind_for_status_check;
ALTER TABLE public.orders
  ADD CONSTRAINT orders_pending_kind_for_status_check
  CHECK (
    (status = 'pending' AND pending_kind IS NOT NULL)
    OR (status IS DISTINCT FROM 'pending' AND pending_kind IS NULL)
  );
...
begin
  ...
  insert into public.orders (
    ...
    preferred_contact_type, preferred_contact_value, status, pending_kind
  )
  values (
    v_order_id, v_user_id, v_customer_name, v_customer_email, v_delivery_method, null,
    v_preferred_type, v_preferred_value, 'pending', 'quote'
  )
...
begin
  ...
  select jsonb_agg(
    jsonb_build_object(
      ...
      -- Legacy keys: unread emailed messages only (photos no longer chip).
...
-- Per-message read_at replaces order-level seen tracking.
DROP FUNCTION IF EXISTS public.mark_my_order_updates_seen(uuid);

-- When any card is in_progress, mark the order in_progress (unless status was
-- edited this save). All-cards-completed → ready still takes precedence.
-- Based on update_order from 20260812033308 (includes whitening damage_tag).
...
begin
  ...
  -- Auto-advance from cards unless order status was edited this save:
  -- all completed → ready; else any in_progress → in_progress.
  if not v_order_status_manually_set then
    ...
    if v_status not in ('ready', 'completed', 'canceled') then
      ...
      ) and v_status is distinct from 'in_progress' then
        update public.orders
        set
          status = 'in_progress',
          pending_kind = null,

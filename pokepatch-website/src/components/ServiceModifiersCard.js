...
export default function ServiceModifiersCard({ panels = [], accent = "mint" }) {
  ...
  function panelClassName(index, total) {
    ...
    if (index > 0) {
      parts.push(
        "border-t border-ink/15 pt-5 sm:border-l sm:border-t-0 sm:pl-4 sm:pt-0"
      );
    }
    if (total > 1) {
      if (index === 0) parts.push("sm:pr-4");
      else if (index === total - 1) parts.push("sm:pr-1");
      else parts.push("sm:pr-4");
    }
    return parts.join(" ");
  }
                ...
                <ul className="mt-2 space-y-1.5 text-sm">
                  {panel.bulk.map((item) => (
                    <li key={item.label}>
                      ...
                      <span className="ml-2 font-bold tabular-nums text-berry">
                        {item.value}
                      </span>
                    </li>
                  ))}
